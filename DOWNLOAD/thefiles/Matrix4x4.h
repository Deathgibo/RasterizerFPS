...
public:
	...
	Vec4d VectorMultiply(Vec4d vec)
	{
		...
		tmp.r = vec.r; tmp.g = vec.g; tmp.b = vec.b;
	...
	void CreateCameraMatrix()//because everything is unit vector their values are between -1 and 1 just like cos and sin so thats why its rotation
	{
		...
		extern Vec3d cameralocation2;
		extern Vec3d cameralookat2;
		extern Vec3d cameraup2;
		extern Vec3d cameraright2;
		extern bool cameratwo;

		extern Vec3d cameralocation3;
		extern Vec3d cameralookat3;
		extern Vec3d cameraup3;
		extern Vec3d cameraright3;
		extern bool camerathree;
		...
		if (cameratwo)
		{
			transform.oneone = 1;				transform.onetwo = 0;			transform.onethree = 0;              transform.onefour = -cameralocation2.x;
			transform.twoone = 0;				transform.twotwo = 1;			transform.twothree = 0;              transform.twofour = -cameralocation2.y;
			transform.threeone = 0;		    transform.threetwo = 0;				transform.threethree = 1;			 transform.threefour = -cameralocation2.z;
			transform.fourone = 0;		   transform.fourtwo = 0;				transform.fourthree = 0;             transform.fourfour = 1;

			rotate.oneone = cameraright2.x;		    rotate.onetwo = cameraright2.y;		 rotate.onethree = cameraright2.z;		rotate.onefour = 0;
			rotate.twoone = cameraup2.x;				rotate.twotwo = cameraup2.y;			 rotate.twothree = cameraup2.z;			rotate.twofour = 0;
			rotate.threeone = cameralookat2.x;		rotate.threetwo = cameralookat2.y;	 rotate.threethree = cameralookat2.z;	rotate.threefour = 0;
			rotate.fourone = 0;						rotate.fourtwo = 0;				     rotate.fourthree = 0;					rotate.fourfour = 1;
		}
		else if (camerathree)
		{
			transform.oneone = 1;				transform.onetwo = 0;			transform.onethree = 0;              transform.onefour = -cameralocation3.x;
			transform.twoone = 0;				transform.twotwo = 1;			transform.twothree = 0;              transform.twofour = -cameralocation3.y;
			transform.threeone = 0;		    transform.threetwo = 0;				transform.threethree = 1;			 transform.threefour = -cameralocation3.z;
			transform.fourone = 0;		   transform.fourtwo = 0;				transform.fourthree = 0;             transform.fourfour = 1;

			rotate.oneone = cameraright3.x;		    rotate.onetwo = cameraright3.y;		 rotate.onethree = cameraright3.z;		rotate.onefour = 0;
			rotate.twoone = cameraup3.x;				rotate.twotwo = cameraup3.y;			 rotate.twothree = cameraup3.z;			rotate.twofour = 0;
			rotate.threeone = cameralookat3.x;		rotate.threetwo = cameralookat3.y;	 rotate.threethree = cameralookat3.z;	rotate.threefour = 0;
			rotate.fourone = 0;						rotate.fourtwo = 0;				     rotate.fourthree = 0;					rotate.fourfour = 1;
		}
		else
		{
	...
	void CreateXRotationMatrixInverse(float theta)
	{
		...
		/*
		oneone = 1;				onetwo = 0;					onethree = 0;               onefour = 0;
		twoone = 0;				twotwo = -cos(angle)/(-pow(sin(angle),2) - pow(cos(angle),2));		twothree = sin(angle) / (-pow(sin(angle), 2) - pow(cos(angle), 2));      twofour = 0;
		threeone = 0;		    threetwo = sin(angle) / (-pow(sin(angle), 2) - pow(cos(angle), 2));		threethree = cos(angle) / (-pow(sin(angle), 2) - pow(cos(angle), 2));    threefour = 0;
		fourone = 0;		    fourtwo = 0;				fourthree = 0;              fourfour = 1;
		*/
	}
